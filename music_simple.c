...
static void draw(void) {
  ...
  for(i = 0; i < ECHANTILLONS/*  - 1 */; i+=2) {
    ...
    for(t = 0; t < 5; t++) {
      gl4dpPutPixel(x0, y0);
      if(x0 + t <= gl4dpGetWidth()-1) gl4dpPutPixel(x0 + t, y0);
      if(y0 + t <= gl4dpGetHeight()-1) gl4dpPutPixel(x0, y0 + t);
      if(x0 + t <= gl4dpGetWidth()-1 && y0 + t <= gl4dpGetHeight()-1) gl4dpPutPixel(x0 + t, y0 + t);

      if(x0 - t >= 0) gl4dpPutPixel(x0 - t, y0);
      if(y0 - t >= 0) gl4dpPutPixel(x0, y0 - t);
      if(x0 - t >= 0 && y0 - t >= 0) gl4dpPutPixel(x0 - t, y0 - t);
    }
  ...
  for(i = 1; i < ECHANTILLONS; i+=2) {
    ...
    //gl4dpSetColor(rand());
    for(t = 0; t < 5; t++) {
      if(y0 - t >= 0 && x0 - t >= 0) gl4dpPutPixel(x0 - t, y0);
      gl4dpPutPixel(x0, y0);
      if(y0 + t <= gl4dpGetHeight()-1 && x0 + t <= gl4dpGetWidth()-1) gl4dpPutPixel(x0 + t, y0);
    }
    //gl4dpLine(x0, y0, x1, y1);
  } 
